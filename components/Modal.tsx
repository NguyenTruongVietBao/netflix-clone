...
const Modal = ({ movie, closeModal }: Props) => {
    ...
    const handleMyList = async () => {
        try {
            ...
            const data = await res.json();
            setUser(data);
            const isMovieInFavorites = data.favorites.find((item: number) => item === movie.id);
            if (isMovieInFavorites) {
                setIsFavorite(true);
                toast.success('Movie has been added to favorites');
            } else {
                setIsFavorite(false);
                toast.error('Movie has been removed from list');
            }
            router.refresh()
        } catch (err) {
